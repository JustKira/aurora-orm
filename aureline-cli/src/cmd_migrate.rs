...
use aureline_migrate::apply::{ApplyOpts, ApplyReport};
use aureline_migrate::config::{self, Config};
...
fn apply_cmd(args: Vec<String>) -> Result<()> {
    ...
    let report = runtime.block_on(aureline_migrate::apply::apply(opts))?;
    print_apply_report(&report);
    Ok(())
}

/// Returns (config, dir-containing-aureline.toml). The dir is needed by the
...
fn print_ops(ops: &[aureline_migrate::ops::Op]) {
    for op in ops {
        if op.destructive() {
...
fn help_text() -> &'static str {
    "aureline migrate commands:\n  \
     ...
     `apply` reads SurrealDB connection from env vars; configure overrides in aureline.toml's\n\
     [database] block. See tools/aureline-migrate/src/apply for the env var name cascade.\n"
}
