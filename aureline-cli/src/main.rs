...
use aureline_core::{check, parse_schema};
...
fn main() -> Result<()> {
    ...
    match command.as_str() {
        ...
        "check" => {
            ...
            println!("OK: {} passed Aureline checks.", path.display());
        }
...
fn required_path(value: Option<String>) -> Result<PathBuf> {
    value
        .map(PathBuf::from)
        .context("missing schema path; pass tools/aureline-examples/schema.aureline")
}

fn check_path(value: Option<String>) -> PathBuf {
    value
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("schema.aureline"))
}
