...
/// One row in the `_aurora_migrations` tracking table on the target DB.
...
/// if missing, then the `_aurora_migrations` tracking table inside them.
...
USE NS {ns} DB {db};
DEFINE TABLE IF NOT EXISTS _aurora_migrations SCHEMAFULL;
DEFINE FIELD IF NOT EXISTS idx        ON _aurora_migrations TYPE int;
DEFINE FIELD IF NOT EXISTS name       ON _aurora_migrations TYPE string;
DEFINE FIELD IF NOT EXISTS checksum   ON _aurora_migrations TYPE string;
DEFINE FIELD IF NOT EXISTS applied_at ON _aurora_migrations TYPE datetime;
",
        ns = conn.namespace,
        db = conn.database,
    );
...
/// Read every row from `_aurora_migrations`. Returns an empty vec if the
/// table exists but is empty.
pub async fn read_applied(client: &Client) -> Result<Vec<AppliedRecord>, ClientError> {
    let mut response = client
        .db()
        .query("SELECT idx, name, checksum, applied_at FROM _aurora_migrations;")
        .await?;
    let rows: Vec<AppliedRecord> = response
        .take(0)
        .map_err(|e| ClientError::Decode(format!("decoding _aurora_migrations: {e}")))?;
    Ok(rows)
}

/// Insert a row into `_aurora_migrations` after a migration runs successfully.
