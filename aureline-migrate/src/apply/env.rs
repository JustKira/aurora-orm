...
use aureline_config::DatabaseConfig;

#[derive(Debug, thiserror::Error)]
pub enum EnvError {
    #[error(
        "missing required env var: tried {0:?}; configure one of these or override via aureline.toml [database]"
    )]
    Missing(Vec<String>),
    #[error("env var {name} (set in aureline.toml) is not defined in the environment")]
...
///   1. `db.env_path` if set in aureline.toml (relative to `config_dir`).
...
/// connection field using either the override name from aureline.toml or the
...
/// `aureline.toml`'s `[database] env_path`) → `<dir>/.env` → `<dir>/.env.local`.
