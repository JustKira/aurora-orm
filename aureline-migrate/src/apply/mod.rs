//! Apply Aureline migrations against a live SurrealDB instance.
//!
...
//! per `aureline.toml`'s `[database]` block, and runs each migration that
//! hasn't yet been recorded in the `_aureline_migrations` tracking table on
//! the target database.
